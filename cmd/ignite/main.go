...
const (
	clientIdentifier = "ignite" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the ignitechain command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2022-2030 The ignitechain Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.RopstenFlag.Name):
		log.Info("Starting ignitechain on Ropsten testnet...")

	case ctx.GlobalIsSet(utils.RinkebyFlag.Name):
		log.Info("Starting ignitechain on Rinkeby testnet...")

	case ctx.GlobalIsSet(utils.GoerliFlag.Name):
		log.Info("Starting ignitechain on Görli testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting ignitechain in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting ignitechain on Ignite mainnet...")
	}
...
func ignite(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		if !ok {
			utils.Fatalf("Ignitechain service not running: %v", err)
		}
